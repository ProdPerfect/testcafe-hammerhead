...
function isCrossDomainXhrWithoutCredentials(ctx) {
  return ctx.req.headers[_internalHeaderNames.default.credentials] === 'false' && ctx.dest.reqOrigin !== ctx.dest.domain;
}

function transformCookieForFetch(src, ctx) {
  const requestCredentials = ctx.req.headers[_internalHeaderNames.default.credentials];
...
const forcedRequestTransforms = {
  [_builtinHeaderNames.default.cookie]: (_src, ctx) => transformCookie(ctx.session.cookies.getHeader(ctx.dest.url) || void 0, ctx),
...
const forcedResponseTransforms = {
  [_builtinHeaderNames.default.setCookie]: (src, ctx) => {
